...
if ($userId != -1) {
    ...
    if (mysqli_num_rows($permissionResult) > 0) {
        if($_POST["newPublishDate"]) {
          if($_POST["newMessage"]) {
            $publishDate = $_POST["newPublishDate"];
            $message = mysqli_real_escape_string($conn, $_POST["newMessage"]);
            $sql = "INSERT INTO principalMessage (userCreated, dateLastUpdated, userLastUpdated, publishDate, message)
                                          VALUES ('$fullEmail', NOW(), '$fullEmail', '$publishDate', '$message')";
            if(mysqli_query($conn, $sql)) {
              logChange($userEmail, $sql, "Principal Admin", "principalMessage");
              $output = "<div class='alert alert-success'>Principal Message Successfully Added!</div>";
            }
          } else {
