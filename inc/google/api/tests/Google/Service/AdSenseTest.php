...
class Google_Service_AdSenseTest extends BaseTest
{
    ...
    public function testAccountsReportGenerate() {
        ...
        $report = $this->adsense->accounts_reports->generate($accountId, $startDate, $endDate, $optParams);
    ...
    public function testAccountsAdUnitsList() {
        ...
        foreach ($accounts['items'] as $account) {
            ...
            foreach ($adClients['items'] as $adClient) {
                $adUnits = $this->adsense->accounts_adunits->listAccountsAdunits($account['id'], $adClient['id']);
    ...
    public function testAccountsCustomChannelsList() {
        ...
        foreach ($accounts['items'] as $account) {
            $adClients = $this->adsense->accounts_adclients->listAccountsAdclients($account['id']);
            foreach ($adClients['items'] as $adClient) {
                $customChannels = $this->adsense->accounts_customchannels->listAccountsCustomchannels($account['id'], $adClient['id']);
    ...
    public function testAccountsUrlChannelsList() {
        ...
        foreach ($accounts['items'] as $account) {
            $adClients = $this->adsense->accounts_adclients->listAccountsAdclients($account['id']);
            foreach ($adClients['items'] as $adClient) {
                $urlChannels = $this->adsense->accounts_urlchannels->listAccountsUrlchannels($account['id'], $adClient['id']);
            ...
            foreach ($adClients['items'] as $adClient) {
                ...
                if (array_key_exists('items', $adUnits)) {
                    foreach ($adUnits['items'] as $adUnit) {
                        $customChannels = $this->adsense->accounts_adunits_customchannels->listAccountsAdunitsCustomchannels($account['id'], $adClient['id'], $adUnit['id']);
                        $this->checkCustomChannelsCollection($customChannels);
            ...
            foreach ($adClients['items'] as $adClient) {
                ...
                if (array_key_exists('items', $customChannels)) {
                    foreach ($customChannels['items'] as $customChannel) {
                        $adUnits = $this->adsense->accounts_customchannels_adunits->listAccountsCustomchannelsAdunits($account['id'], $adClient['id'], $customChannel['id']);
                        $this->checkAdUnitsCollection($adUnits);
                        // it's too expensive to go through each, if one is correct good
                        break 3;
                    }
    ...
    public function testAdUnitsCustomChannelsList() {
        ...
        foreach ($adClients['items'] as $adClient) {
            ...
            if (array_key_exists('items', $adUnits)) {
                foreach ($adUnits['items'] as $adUnit) {
                    $customChannels = $this->adsense->adunits_customchannels->listAdunitsCustomchannels($adClient['id'], $adUnit['id']);
                    $this->checkCustomChannelsCollection($customChannels);
                    break 2;
                }
            }
    ...
    public function testCustomChannelsAdUnitsList() {
        ...
        foreach ($adClients['items'] as $adClient) {
            ...
            if (array_key_exists('items', $customChannels)) {
                foreach ($customChannels['items'] as $customChannel) {
                    $adUnits = $this->adsense->customchannels_adunits->listCustomchannelsAdunits($adClient['id'], $customChannel['id']);
                    $this->checkAdUnitsCollection($adUnits);
                    break 2;
                }
            }
    ...
    private function getReportOptParams() {
        return array('metric' => array('PAGE_VIEWS', 'AD_REQUESTS'), 'dimension' => array('DATE', 'AD_CLIENT_ID'), 'sort' => array('DATE'), 'filter' => array('COUNTRY_NAME==United States'),);
    }
