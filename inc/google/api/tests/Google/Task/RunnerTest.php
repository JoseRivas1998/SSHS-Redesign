...
class Google_Task_RunnerTest extends BaseTest
{
     ...
     * @expectedException Google_Service_Exception
     */
    public function testMultipleRestRetriesWithErrors($errorCode, $errorBody = '{}') {
    ...
    public function testOneRestRetryWithSuccess($errorCode, $errorBody = '{}') {
        $this->setRetryConfig(array('retries' => 1));
        $result = $this->setNextResponse($errorCode, $errorBody)->setNextResponse(200, '{"success": true}')->makeRequest();
     ...
     * @dataProvider defaultRestErrorProvider
     */
    public function testMultipleRestRetriesWithSuccess($errorCode, $errorBody = '{}') {
        $this->setRetryConfig(array('retries' => 5));
        $result = $this->setNextResponses(2, $errorCode, $errorBody)->setNextResponse(200, '{"success": true}')->makeRequest();
     ...
     * @expectedException Google_Service_Exception
     */
    public function testCustomRestRetryMapReplacesDefaults($errorCode, $errorBody = '{}') {
    ...
    public function testCustomRestRetryMapAddsNewHandlers() {
        $this->setRetryMap(array('403' => Google_Task_Runner::TASK_RETRY_ALWAYS));

        $this->setRetryConfig(array('retries' => 5));
        $result = $this->setNextResponses(2, 403)->setNextResponse(200, '{"success": true}')->makeRequest();
    ...
    public function testCustomRestRetryMapWithCustomLimits($limit) {
        $this->setRetryMap(array('403' => $limit));
    ...
    public function testRestTimeouts($config, $minTime) {
        ...
        $this->setNextResponses($config['retries'], 500)->setNextResponse(200, '{"success": true}');
     ...
     * @expectedException Google_Service_Exception
     */
    public function testMultipleCurlRetriesWithErrors($errorCode, $errorMessage = '') {
        $this->setRetryConfig(array('retries' => 5));
    ...
    public function testOneCurlRetryWithSuccess($errorCode, $errorMessage = '') {
        $this->setRetryConfig(array('retries' => 1));
        $result = $this->setNextResponseThrows($errorMessage, $errorCode)->setNextResponse(200, '{"success": true}')->makeRequest();
     ...
     * @dataProvider defaultCurlErrorProvider
     */
    public function testMultipleCurlRetriesWithSuccess($errorCode, $errorMessage = '') {
        $this->setRetryConfig(array('retries' => 5));
        $result = $this->setNextResponsesThrow(2, $errorMessage, $errorCode)->setNextResponse(200, '{"success": true}')->makeRequest();
     ...
     * @expectedException Google_Service_Exception
     */
    public function testCustomCurlRetryMapReplacesDefaults($errorCode, $errorMessage = '') {
        $this->setRetryMap(array());
    ...
    public function testCustomCurlRetryMapAddsNewHandlers() {
        $this->setRetryMap(array(CURLE_COULDNT_RESOLVE_PROXY => Google_Task_Runner::TASK_RETRY_ALWAYS));

        $this->setRetryConfig(array('retries' => 5));
        $result = $this->setNextResponsesThrow(2, '', CURLE_COULDNT_RESOLVE_PROXY)->setNextResponse(200, '{"success": true}')->makeRequest();
    ...
    public function testCustomCurlRetryMapWithCustomLimits($limit) {
        $this->setRetryMap(array(CURLE_COULDNT_RESOLVE_PROXY => $limit));

        $this->setRetryConfig(array('retries' => 5));
        $this->setNextResponsesThrow($limit + 1, '', CURLE_COULDNT_RESOLVE_PROXY)->makeRequest();
    }
    ...
    public function testCurlTimeouts($config, $minTime) {
        $this->setRetryConfig($config);
        $this->setNextResponsesThrow($config['retries'], '', CURLE_GOT_NOTHING)->setNextResponse(200, '{"success": true}');
    ...
    public function testTaskRetryOffByDefault() {
        $this->setNextTaskAllowedRetries(Google_Task_Runner::TASK_RETRY_ALWAYS)->runTask();
    }
    ...
    public function testOneTaskRetryWithError() {
        $this->setRetryConfig(array('retries' => 1));
        $this->setNextTasksAllowedRetries(2, Google_Task_Runner::TASK_RETRY_ALWAYS)->runTask();
    }
    ...
    public function testMultipleTaskRetriesWithErrors() {
        $this->setRetryConfig(array('retries' => 5));
        $this->setNextTasksAllowedRetries(6, Google_Task_Runner::TASK_RETRY_ALWAYS)->runTask();
    }

    public function testOneTaskRetryWithSuccess() {
        $this->setRetryConfig(array('retries' => 1));
        $result = $this->setNextTaskAllowedRetries(Google_Task_Runner::TASK_RETRY_ALWAYS)->setNextTaskReturnValue('success')->runTask();
    ...
    public function testMultipleTaskRetriesWithSuccess() {
        $this->setRetryConfig(array('retries' => 5));
        $result = $this->setNextTasksAllowedRetries(2, Google_Task_Runner::TASK_RETRY_ALWAYS)->setNextTaskReturnValue('success')->runTask();
    ...
    public function testTaskRetryWithCustomLimits($limit) {
        $this->setRetryConfig(array('retries' => 5));
        $this->setNextTasksAllowedRetries($limit + 1, $limit)->runTask();
    }
    ...
    public function testTaskTimeouts($config, $minTime) {
        $this->setRetryConfig($config);
        $this->setNextTasksAllowedRetries($config['retries'], $config['retries'] + 1)->setNextTaskReturnValue('success');
    ...
    public function customLimitsProvider() {
        return array(array(Google_Task_Runner::TASK_RETRY_NEVER), array(Google_Task_Runner::TASK_RETRY_ONCE),);
    }
    ...
    public function badTaskConfigProvider() {
        return array(array(array('initial_delay' => -1), 'must not be negative'), array(array('max_delay' => 0), 'must be greater than 0'), array(array('factor' => 0), 'must be greater than 0'), array(array('jitter' => 0), 'must be greater than 0'), array(array('retries' => -1), 'must not be negative'));
    }
    ...
    public function defaultRestErrorProvider() {
        return array(array(500), array(503), array(403, '{"error":{"errors":[{"reason":"rateLimitExceeded"}]}}'), array(403, '{"error":{"errors":[{"reason":"userRateLimitExceeded"}]}}'),);
    }
    ...
    public function defaultCurlErrorProvider() {
        return array(array(6),  // CURLE_COULDNT_RESOLVE_HOST
     ...
     */
    public static function assertTaskTimeGreaterThanOrEqual($expected, $callback, $delta = 0.0) {
        $time = microtime(true);
    ...
    private function setRetryConfig(array $config) {
        $config += array('initial_delay' => .0001, 'max_delay' => .001, 'factor' => 2, 'jitter' => .5, 'retries' => 1);
     ...
     * @return TaskTest
     */
    private function setNextResponses($count, $code = '200', $body = '{}', array $headers = array()) {
        while ($count-- > 0) {
     ...
     * @return TaskTest
     */
    private function setNextResponse($code = '200', $body = '{}', array $headers = array()) {
        $this->mockedCalls[$this->mockedCallsCount++] = array('code' => (string)$code, 'headers' => $headers, 'body' => is_string($body) ? $body : json_encode($body));
    ...
    private function setNextResponseThrows($message, $code) {
        $this->mockedCalls[$this->mockedCallsCount++] = new Google_Service_Exception($message, $code, null, array());
    ...
    private function makeRequest() {
        ...
        if ($this->isGuzzle5()) {
            $http->expects($this->exactly($this->mockedCallsCount))->method('createRequest')->will($this->returnValue(new GuzzleHttp\Message\Request('GET', '/test')));
        }
    ...
    private function runTask() {
        ...
        $exception = $this->getMockBuilder('Google_Service_Exception')// HHVM blows up unless this is set
            // @see https://github.com/sebastianbergmann/phpunit-mock-objects/issues/207
            ->setMethods(array('setTraceOptions'))->disableOriginalConstructor()->getMock();
        $exceptionCount = 0;
