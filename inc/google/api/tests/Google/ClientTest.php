...
class Google_ClientTest extends BaseTest
{
    ...
    public function testSignAccessToken() {
        ...
        $http = new Client();
        $client->setAccessToken(['access_token' => 'test_token', 'expires_in' => 3600, 'created' => time(),]);
    ...
    public function testCreateAuthUrl() {
        ...
        $expected = "https://accounts.google.com/o/oauth2/auth" . "?response_type=code" . "&access_type=offline" . "&client_id=clientId1" . "&redirect_uri=http%3A%2F%2Flocalhost" . "&state=xyz" . "&scope=http%3A%2F%2Fgoogleapis.com%2Fscope%2Ffoo" . "&approval_prompt=force" . "&login_hint=bob%40example.org";
        ...
        $authUrl = $client->createAuthUrl("http://googleapis.com/scope/foo");
        $expected = "https://accounts.google.com/o/oauth2/auth" . "?response_type=code" . "&access_type=offline" . "&client_id=clientId1" . "&redirect_uri=http%3A%2F%2Flocalhost" . "&state=xyz" . "&scope=http%3A%2F%2Fgoogleapis.com%2Fscope%2Ffoo" . "&hd=example.com" . "&include_granted_scopes=true" . "&openid.realm=example.com" . "&prompt=select_account";
    ...
    public function testPrepareService() {
        ...
        $this->assertEquals('' . 'https://accounts.google.com/o/oauth2/auth' . '?response_type=code' . '&access_type=online' . '&client_id=test1' . '&redirect_uri=http%3A%2F%2Flocalhost%2F' . '&state=xyz' . '&scope=http%3A%2F%2Ftest.com%20scope2' . '&approval_prompt=auto',
    ...
    public function testRefreshTokenSetsValues() {
        $token = json_encode(array('access_token' => 'xyz', 'id_token' => 'ID_TOKEN',));
        $postBody = $this->getMock('Psr\Http\Message\StreamInterface');
        $postBody->expects($this->once())->method('__toString')->will($this->returnValue($token));
        $response = $this->getMock('Psr\Http\Message\ResponseInterface');
        $response->expects($this->once())->method('getBody')->will($this->returnValue($postBody));
        $http = $this->getMock('GuzzleHttp\ClientInterface');
        $http->expects($this->once())->method('send')->will($this->returnValue($response));
